...
function CartModal({ cart, paciente, tipoCotizacion, cuponInput, cuponDescuento, onChangePaciente, onChangeQty, onAddCustomItem, onChangeItemName, onRemoveItem, onChangePrice, onApplyCupon, onClose, onClearAll, onExportPDF }) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50 px-2 sm:px-4">
      <div className="bg-white rounded-lg shadow-lg p-4 sm:p-6 w-full max-w-md flex flex-col mt-10 sm:mt-16 max-h-[90vh] overflow-y-auto">
        ...
        {cart.length === 0 ? (
          ...
          <div className="space-y-2 mb-4 max-h-64 overflow-y-auto border border-gray-200 rounded-lg p-2 bg-gray-50">
            {cart.map((item) => (
              <div key={item.id} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 bg-white rounded p-2 shadow">
                <div className="flex flex-col min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {item.type === "custom" ? (
                      <input
                        type="text"
                        className="border rounded px-2 py-1 text-sm font-semibold text-blue-700 w-full"
                        placeholder="Nombre de la prueba"
                        value={item.name}
                        onChange={e => onChangeItemName(item.id, e.target.value.toUpperCase())}
                      />
                    ) : (
                      <span className="font-semibold text-blue-700 break-words">{item.name.toUpperCase()}</span>
                    )}
                  </div>
        ...
        <div className="flex flex-col sm:flex-row sm:flex-wrap gap-2 mt-2">
          <button
            className="w-full sm:w-auto px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700 font-semibold text-sm sm:text-base"
            onClick={onExportPDF}
          >Exportar PDF</button>
          <button
            className="w-full sm:w-auto px-4 py-2 bg-[#25D366] text-white rounded hover:bg-[#128C7E] font-semibold flex items-center justify-center gap-2 text-sm sm:text-base"
          ...
          <button
            className="w-full sm:w-auto px-4 py-2 bg-gray-400 text-white rounded hover:bg-gray-500 font-semibold text-sm sm:text-base"
            onClick={onClearAll}
          >Limpiar todo</button>
          <button
            className="w-full sm:w-auto px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 font-semibold text-sm sm:text-base"
            onClick={onClose}
          >Cerrar</button>
        </div>
      </div>
